...
def kampanya_onerileri_uret():
    ...
    # Cross-Sell üretimi ağır (association rules) ve ayrı bir süreçte yönetilmeli.
    ...
    try:
        ...
        for row in associations:
            try:
                ...
                # Lift bazlı dinamik dönüşüm oranı (Kitle Bazlı vs Organik Bazlı Hızlandırma)
                ...
                tahmini_katilim = max(1, min(round(max(kitle_bazli_katilim, organik_bazli_katilim)), hedef_musteri))
    ...
    try:
        ...
        # Segment-specific SQL templates with different behavioral ORDER BY signals:
        # - Kayip Musteriler: "son veda" — categories bought most recently before going silent
        # - Uyuyanlar: "nostalji" — categories bought frequently in past but not recently
        # - Risk Altindakiler: "bağlayıcı" — categories with highest repeat purchase rate
        # - Ilgi Bekleyenler: fallback popular (different products via global dedup)
        def _get_winback_products(cursor, ph, seg_numbered, seg_display, is_pg):
            if seg_display == 'Kayip Musteriler':
                # Order by: most recently purchased category before they went silent
                if is_pg:
                    date_expr = "CURRENT_DATE - INTERVAL '180 days'"
                else:
                    date_expr = "date('now', '-180 days')"
                cursor.execute(f"""
                    WITH churn_custs AS (
                ...
                # Order by: nostalgia score = high historical revenue but low recent (last 90d) revenue
                if is_pg:
                    recent_date = "CURRENT_DATE - INTERVAL '90 days'"
                else:
                    recent_date = "date('now', '-90 days')"
                cursor.execute(f"""
                    WITH churn_custs AS (
                        SELECT id FROM musteriler WHERE rfm_segment = {ph}
                    ),
                ...
                # Order by: repeat purchase rate = total transactions / distinct customers (sticky categories)
                cursor.execute(f"""
                    WITH churn_custs AS (
                        SELECT id FROM musteriler WHERE rfm_segment = {ph}
                    ),
                ...
                # Ilgi Bekleyenler: fallback to revenue-based, global dedup ensures different products
                cursor.execute(f"""
                    WITH churn_custs AS (
                        SELECT id FROM musteriler WHERE rfm_segment = {ph}
                    ),
        ...
        for seg_numbered, info in segment_info.items():
            ...
            for p in products:
                ...
                if ana and ana in used_categories_per_seg[seg_numbered]:
                    continue  # Bu segment için bu ana kategori zaten kullanıldı
                ...
                # Segment-specific win-back parametreleri ve davranışsal mesajlar
                ...
                if seg_disp == 'Kayip Musteriler':
                    ...
                    beklenen_wb = f"İlgi bekleyen müşterilerin alışveriş sıklığını artırmak ve churn yolculuğunu durdurmak."
                ...
                oneriler.append({
                    ...
                    'UrunID': db_engine.val(p, 'urun_id'),
                    'UrunAdi': urun_ad,
                    ...
                    'VeriOzeti': (
                        f"Müşteri Sayısı: {info['count']}, Ort. Churn Riski: %{info['avg_risk']:.0f}, "
                        f"Kategori: {ana}, Ürün Cirosu: {ciro:.0f} TL, "
                        f"Ürün Müşteri Sayısı: {musteri_count}"
                    ),
                    'BeklenenSonuc': beklenen_wb,
                    'KaynakKategoriAd': f"{seg_disp}|{ana}"  # Unique per segment+category to avoid ON CONFLICT clash
                })
                if urun_id_p:
    ...
    try:
        ...
        # Her segment kendi içinde kategori çeşitliliği (en fazla 1 ürün/ana kategori) uygular.
        # Segment adları segmenturuntercihleri tablosundaki gerçek değerlerle eşleşmeli.
        # NOT: tabloda 'Sadiklar' var, 'Sadik Musteriler' değil.
        LOYALTY_SEGMENT_STRATEGIES = [
            # (db_name_in_segmenturuntercihleri, order_expr, indirim, sure, oncelik)
            ('Sampiyonlar',             'penetrasyon * -1',   15.0, 14, 1),  # en yaygın sevdikleri
            ('Sadiklar',                'penetrasyon * -1',   15.0, 14, 1),  # en yaygın sevdikleri
            ('Yuksek Harcama Yapanlar', 'toplam_ciro * -1',   10.0, 14, 1),  # en yüksek ciro
            ('Potansiyel Sampiyonlar',  'toplam_ciro * -1',   20.0, 21, 2),  # en popüler
            ('Tekrar Kazanilanlar',     'toplam_ciro * -1',   30.0, 21, 2),  # en popüler
            ('Yeni Musteriler',         'toplam_ciro * -1',   25.0, 30, 2),  # en popüler giriş
            ('Sadik Olmaya Adaylar',    'toplam_ciro * -1',   20.0, 30, 3),  # en yüksek ciro
            ('Ilgi Bekleyenler',        'toplam_ciro * -1',   25.0, 30, 3),  # en yüksek ciro
        ]
        ...
        for seg_name_db, order_expr, indirim, sure, oncelik in LOYALTY_SEGMENT_STRATEGIES:
            rows = _get_loyalty_products_for_segment(
                ...
                excluded_ids=set(),  # Segment arası dedup yok — her segment bağımsız
                limit=20             # Kategori çeşitliliği için geniş havuz
            )
            seg_added = 0
            used_cats_this_seg = set()  # Segment içi ana kategori dedup
            for r in rows:
                if seg_added >= 3:
                    break
                urun_id_l = db_engine.val(r, 'urun_id')
                kat_ad = db_engine.val(r, 'kategori_ad', '') or ''
                # Ana kategori: ilk ' > ' öncesi veya tüm string
                ana_kat = kat_ad.split(' > ')[0].strip() if ' > ' in kat_ad else kat_ad
                if ana_kat and ana_kat in used_cats_this_seg:
                    continue  # Bu segmente bu ana kategoriden zaten bir ürün aldık

                segment = seg_name_db  # normalized ASCII name (matches segmenturuntercihleri)

                # Gerçek müşteri sayısı: musteriler tablosundan (normalize eşleşme)
                gercek_musteri = (
                    segment_musteri_map.get(segment, 0) or
                ...
                # Segment-specific messaging — her segmentin gerçek davranışına dayalı
    ...
    # Batch insert to avoid timeout — deadlock retry ile
        ...
        for attempt in range(3):
            try:
                ...
                if 'deadlock' in str(e).lower() and attempt < 2:
                    logger.warning(f"Deadlock at {i}, retry {attempt+1}/3...")
    ...
    # kaynak_marka_id ve hedef_marka_id kolonlarını güncelle
    logger.info("Marka ID'leri (kaynak/hedef) kampanya önerilerine ekleniyor...")
    try:
        cursor.execute("""
        ...
        cursor.execute("""
            ...
            WHERE oneri_durumu = 'Bekliyor'
              AND kategori_ad IS NOT NULL
        """)
        conn.commit()
        logger.info("Marka ID'leri güncellendi.")
    except Exception as e:
        logger.warning(f"Marka ID eşleştirme hatası: {e}")
        conn.rollback()
